...
public class SpatialKDTree : IKDTree
{
    public KDNode root;
    ...
    public SpatialKDTree(List<TriangleInfo> triangleList)
    {
    ...
    private KDNode Build(List<TriangleInfo> tris, int depth)
    {
        ...
        // Classical KD-Tree: space-based partitioning, duplicates allowed for overlapping triangles
        foreach (var tri in tris)
        {
            float triMin = tri.bounds.min[axis];
            float triMax = tri.bounds.max[axis];

            // If triangle spans the split plane, put it in both
            if (triMin <= node.splitPosition && triMax >= node.splitPosition)
            {
                leftTris.Add(tri);
                rightTris.Add(tri);
            }
            else if (triMax < node.splitPosition)
            {
                leftTris.Add(tri);
            }
            else
